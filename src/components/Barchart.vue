<template>
  <Card
    class="max-w-sm w-full bg-white rounded-lg shadow dark:bg-gray-800 p-4 md:p-6 h-fit"
  >
    <template #header>
      <div
          ...
          <span
            ...
            :class="{
              'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300':
                data?.last_7_days?.change < 0,
            }"
          >
            <svg
              class="w-2.5 h-2.5 me-1.5"
              :class="{ 'rotate-180': data?.last_7_days?.change < 0 }"
              aria-hidden="true"
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 10 14"
            ...
            {{ data?.last_7_days?.change?.toFixed(0) ?? "-" }} %
          </span>
        </div>
      </div>
      <div class="grid grid-cols-2">
        ...
        <dl class="flex items-center justify-end">
          ...
          <dd class="text-gray-900 text-sm dark:text-white font-semibold">
            {{ data?.last_7_days?.utilization_rate?.toFixed(1) ?? "-" }} %
          </dd>
        </dl>
      </div>
    </template>
...
onMounted(() => {
  if (
    ...
    document.getElementById("column-chart")?.childElementCount === 0
  ) {
