...
function checkWIFI()
    if wifiTrys > NUMWIFITRYS then
        ...
        if (ipAddr ~= nil) and (ipAddr ~= "0.0.0.0") then
            ...
            tmr.create():alarm(500, 0, launch)
        else
            -- Reset alarm again
            tmr.create():alarm(2500, 0, checkWIFI)
...
if (ipAddr == nil) or (ipAddr == "0.0.0.0") then
    ...
    wifi.sta.config({ ssid = SSID, pwd = APPWD })
    print("Waiting for connection")
    tmr.create():alarm(2500, 0, checkWIFI) -- Call checkWIFI 2.5S in the future.
else
